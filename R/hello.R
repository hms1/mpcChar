execute <- function(connectionDetails,
  ...
  if(getTreatmentInfo){
    ...
    generate_survival_crpc(formatted_regimens, count_mask = minCount) %>%
      readr::write_csv(file.path(outputFolder, "survival_mcrpc.csv"))

    generate_survival_bcr(connection, cohortDatabaseSchema, cdmDatabaseSchema, regimenIngredientsTable, cohortTable, count_mask = minCount) %>%
